...
const freeTierClient = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
const gcpConsoleClient = new GoogleGenAI({ apiKey: process.env.GOOGLE_API_KEY });

/**
 * Executes a generative content request against the prioritized client tier chain.
 * Cascades down to the Google Cloud key if the free tier experiences an exhaustion failure.
 */
async function generateContentWithFallback(requestPayload: any) {
  try {
    if (!process.env.GEMINI_API_KEY) {
      throw new Error("Free Tier Key (GEMINI_API_KEY) missing, skipping to backup tier.");
    }
    return await freeTierClient.models.generateContent(requestPayload);
  } catch (freeTierError: any) {
    console.warn("⚠️ Primary Free Tier Client failed/exhausted. Falling back to GCP Console API key...", freeTierError.message);
    
    try {
      if (!process.env.GOOGLE_API_KEY) {
        throw new Error("GCP backup key environment variable 'GOOGLE_API_KEY' is not configured.");
      }
      return await gcpConsoleClient.models.generateContent(requestPayload);
    } catch (gcpError: any) {
      console.error("❌ Both API client tier authentication contexts failed execution:", gcpError.message);
      throw gcpError;
    }
  }
}
...
app.post("/api/analyze-telemetry", async (req, res) => {
  try {
    ...
    // 2. IMUTABLE KEY GENERATION: Generate the composite cache key using these freshly binned raw values BEFORE any conditional overrides alter the data pipeline
    const metroSurge = surgeRate;
    const cacheKey = `${binnedGateC}-${binnedGateD}-${metroSurge}-${fanContext}`;
...
`,
      ...
      response = await generateContentWithFallback(requestPayload);
